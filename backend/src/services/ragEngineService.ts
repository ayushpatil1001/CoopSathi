...
export class RAGEngineService {
  private knowledgeCorpus: RAGChunk[] = [
    // 1. Multi-State Co-operative Societies Act, 2023 - Voting Rights
    {
      ...
      keywords: ['vote', 'voting', 'election', 'agm', 'active member', 'rights', 'right', 'मतदान', 'हक्क', 'अधिकार', 'निवडणूक', 'चुनाव', 'सभासद', 'सदस्य'],
    ...
    // 2. Multi-State Co-operative Societies Act, 2023 - Election Authority
    {
      ...
      keywords: ['election', 'authority', 'board', 'director', 'democracy', 'निवडणूक', 'प्राधिकरण', 'चुनाव'],
    ...
    // 3. Multi-State Co-operative Societies Act, 2023 - Ombudsman & Grievances
    {
      ...
      keywords: ['ombudsman', 'grievance', 'complaint', 'fraud', 'deposit', 'dividend', 'refusal', 'तक्रार', 'शिकायत', 'ओम्बड्समन', 'ठेव', 'लाभांश'],
    ...
    // 4. Multi-State Co-operative Societies Act, 2023 - Right to Information
    {
      ...
      keywords: ['audit', 'balance sheet', 'inspect', 'accounts', 'rti', 'transparency', 'हिशोब', 'तपासणी', 'ऑडिट'],
    ...
    // 5. PMFBY - 72-Hour Calamity Intimation Rule
    {
      ...
      keywords: ['pmfby', 'crop insurance', '72 hour', '72 hours', 'intimation', 'loss', 'hailstorm', 'flood', 'विमा', 'पीक विमा', '७२ तास', 'नुकसान', 'गारपीट', 'पूर', 'पाऊस'],
    ...
    // 6. PMFBY - Subsidized Farmer Premium Rates
    {
      ...
      keywords: ['premium', 'rate', 'kharif', 'rabi', 'commercial', 'subsidy', 'हप्ता', 'खरीप', 'रब्बी', 'दर', 'अनुदान'],
      officialUrl: 'https://pmfby.gov.in',
    ...
    // 8. PACS Modernization & Model By-laws - 25+ Activities
    {
      ...
      keywords: ['pacs', 'by-law', 'bylaw', 'model', 'jan aushadhi', 'custom hiring', 'drone', 'पैक्स', 'पॅक्स', 'जन औषधी', 'खत', 'यंत्रे'],
    ...
    // 9. PACS Computerization Mission (79,630 Target & ERP)
    {
      ...
      keywords: ['computerization', 'erp', 'cloud', 'digital', 'dccb', 'संगणकीकरण', 'ई-पॅक्स', 'सॉफ्टवेअर'],
    ...
    // 10. Financial Literacy - Kisan Credit Card (KCC) 4% Rate
    {
      ...
      keywords: ['kcc', 'kisan credit card', 'loan', 'interest', '4%', 'prompt repayment', 'subvention', 'कर्ज', 'व्याज', 'ब्याज', 'केसीसी', 'बचत'],
  ...
  public retrieve(query: string, topK: number = 3): RAGRetrievalResult[] {
    ...
    const scored = this.knowledgeCorpus.map((chunk) => {
      ...
      if (lowerQuery.includes('72') && chunk.id === 'PMFBY-CLAUSE21') score += 10;
      if ((lowerQuery.includes('vote') || lowerQuery.includes('मतदान') || lowerQuery.includes('हक्क')) && chunk.id === 'MSCS-SEC29') score += 10;
      if ((lowerQuery.includes('kcc') || lowerQuery.includes('interest') || lowerQuery.includes('व्याज')) && chunk.id === 'KCC-MISS-SUBVENTION') score += 8;
      if ((lowerQuery.includes('pacs') || lowerQuery.includes('पैक्स') || lowerQuery.includes('पॅक्स')) && chunk.id.includes('PACS')) score += 6;
      if ((lowerQuery.includes('ombudsman') || lowerQuery.includes('complaint') || lowerQuery.includes('तक्रार')) && chunk.id === 'MSCS-SEC85') score += 10;
